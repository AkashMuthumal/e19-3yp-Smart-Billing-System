...
export async function getBillsToday(request, response) {
    try {
        ...
        response
            .status(200)
            .json({ billCountToday, billCountYesterday, percentageChange });
    } catch (error) {
        console.error(error.message);
        res.status(500).json({
            error: "An error occurred while fetching bill count.",
        });
    }
}
...
export async function getBillCountPerdate(request, response) {
    try {
        ...
        response
            .status(500)
            .json({
                error: "An error occurred while fetching the bill count for the selected day.",
            });
    }
}
